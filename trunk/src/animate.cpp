...
bool wxAnimationPlayer::PlayFrame()
{
    ...
    PlayFrame(GetCurrentFrame(), * GetWindow(), pt);
...
wxRect wxGIFAnimation::GetFrameRect(int i) const
{
    ...
    if(i==0) {
      return wxRect(0,0,m_decoder->GetWidth(),m_decoder->GetHeight());
    }
...
bool wxGIFAnimation::LoadData(char* data,int size)
{
  if (m_decoder)
    delete m_decoder;
  m_decoder = NULL;
...
bool wxAnimationCtrlBase::LoadData(char* data,int size)
{
    if (m_animationPlayer.IsPlaying())
        m_animationPlayer.Stop();
    ...
    if (!m_animation->LoadData(data,size) || !m_animation->IsValid())
    {
        delete m_animation;
        m_animation = NULL;
        return FALSE;
