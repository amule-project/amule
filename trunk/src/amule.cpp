...
void CamuleApp::OnlineSig(bool zero /* reset stats (used on shutdown) */)
{
	...
	sprintf(buffer, "%llu", (long long unsigned int)(stat_sessionReceivedBytes+ thePrefs::GetTotalDownloaded()));
	amulesig_out.Write(buffer, strlen(buffer));
	amulesig_out.Write("\n",1);

	// Total sent in bytes
	sprintf(buffer, "%llu", (long long unsigned int)(stat_sessionSentBytes+ thePrefs::GetTotalUploaded()));
	amulesig_out.Write(buffer, strlen(buffer));
	amulesig_out.Write("\n",1);
	...
	if (zero) {
		...
		sprintf(buffer, "%llu", (long long unsigned int)stat_sessionReceivedBytes);
        	amulesig_out.Write(buffer, strlen(buffer));
        	amulesig_out.Write("\n",1);
	}
	...
	if (zero) {
		...
		sprintf(buffer, "%llu", (long long unsigned int)stat_sessionSentBytes);
        	amulesig_out.Write(buffer, strlen(buffer));
        	amulesig_out.Write("\n",1);
	}
...
void CamuleApp::ComputeSessionAvg(float& kBpsSession, float& kBpsCur, double& kBytesTrans, double& sCur, double& sTrans)
{
...
void CamuleApp::ComputeRunningAvg(float& kBpsRunning, float& kBpsSession, double& kBytesTrans, 
...
unsigned CamuleApp::GetHistory(  // Assemble arrays of sample points for a graph
...
unsigned CamuleApp::GetHistoryForWeb(  // Assemble arrays of sample points for the webserver
...
void CamuleApp::ComputeAverages(
...
void CamuleApp::VerifyHistory(bool bMsgIfOk)
