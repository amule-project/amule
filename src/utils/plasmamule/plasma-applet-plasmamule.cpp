	...
	const QStyleOptionGraphicsItem *option, const QRect &contentsRect)
{
	...
	} else if (m_uptime == 0)
	{
		...
		runtime = QTime (hours, minutes, time).toString(Qt::LocalDate);
		if (days)
		{
		...
		if (painter->boundingRect(contentsRect, Qt::TextDontClip | Qt::AlignTop | Qt::AlignLeft | Qt::TextWordWrap,
			 message).width() > contentsRect.width())
		{
			resize(painter->boundingRect(contentsRect, message).width()+(contentsRect.topLeft().x()*2),
				painter->boundingRect(contentsRect, message).width()+(contentsRect.topLeft().x()*2));
		} else if (painter->boundingRect(contentsRect, Qt::TextDontClip | Qt::AlignTop | Qt::AlignLeft | Qt::TextWordWrap,
...
void PlasmaMuleApplet::connectToEngine()
{
	...
	m_aMuleEngine->connectAllSources(this, m_update_interval);
...
{
	m_aMuleEngine->connectSource(source, this, m_update_interval);
}
...
void PlasmaMuleApplet::dataUpdated(const QString& source, const Plasma::DataEngine::Data &data)
{
	...
	{
		if (data["update_interval"] != m_update_interval && data.contains("update_interval"))
		{
			m_update_interval = data["update_interval"].toInt();
			connectToEngine ();
		}
	} else {
