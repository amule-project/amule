...
CECPacket * CamulecmdApp::createRequest( ec_tagname_t name, const wxString & data ) const {
   CECPacket * request = new CECPacket( name );
   request->AddTag( EC_TAG_STRING, data );
   return request;
}

CECPacket * CamulecmdApp::createRequest( ec_tagname_t name, const CECTag & tag ) const {
   CECPacket * request = new CECPacket( name );
   request->AddTag( tag );
   return request;
}

void CamulecmdApp::addConnectRequest( const wxString & args, std::list<CECPacket *> & request_list ) const {
   if ( args.IsEmpty()) {
      ...
      addr.m_ip[3] = ip[3];
      addr.m_port = port;
      request_list.push_back( createRequest( EC_OP_SERVER_CONNECT, CECTag( EC_TAG_SERVER, addr )));
   }
}

void CamulecmdApp::addSearchRequest( const wxString & args, EC_SEARCH_TYPE search_type, std::list<CECPacket *> & request_list ) const {
   if( ! args.IsEmpty()) {
      ...
      if (args.Find('[') == 0) {
         ...
         if(( end != wxNOT_FOUND )&&( comma > 0 )&&( comma < end )) {
            const wxString arg_type = args.substr( 1, comma - 1 );
            if( arg_type == "type" ) {
...
void CamulecmdApp::addRenameRequest( const wxString & argsIn, std::list<CECPacket *> & request_list ) const {
   if( argsIn.IsEmpty() ) {
      Show(_("This command requires two arguments: hash of partfile and a new name.\n"));
   }
   else {
      wxString args = argsIn;
      args = args.Trim( true ).Trim( false );
      int sep = args.find( ' ' );
      const int tab = args.find( '\t' );
      if(( sep == -1 )||(( tab > 0 )&&( tab < sep ))) {
         sep = tab;
      }
      if( sep > 0 ) {
         const wxString hashStr         = args.substr( 0, sep );
         const wxString newPartfileName = args.substr( sep + 1 );
         if( newPartfileName.Length() <= 3 ) {
            Show(_("This command requires two arguments: hash of partfile and a new name.\n"));
            Show(_("New name too short.\n"));
         }
         else {
            CMD4Hash hash;
            if( hash.Decode( hashStr ) &&( ! hash.IsEmpty())) {
               const wxCharBuffer newNameBuffer( unicode2char( newPartfileName ));
               const char *       newName = (const char *)newNameBuffer;
               CECPacket *        request = new CECPacket( EC_OP_RENAME_FILE );
               ...
               Show(_("This command requires two arguments: hash of partfile and a new name.\n"));
               fprintf( stderr, "'%s' is not a valid hash\n", (const char *)hashStr.c_str());
            }
         }
      }
      else {
         Show(_("This command requires two arguments: id, a number, and new name.\n"));
         fprintf( stderr, "sep <= 0 in '%s'\n", (const char *)args.c_str());
      }
   }
}

void CamulecmdApp::addGetIpFilterRequest( std::list<CECPacket *> & request_list ) const {
   CECPacket * request = new CECPacket( EC_OP_GET_PREFERENCES );
   request->AddTag( CECTag( EC_TAG_SELECT_PREFS, (uint32)EC_PREFS_SECURITY ));
   request_list.push_back( request );
}

void CamulecmdApp::addSetIpFilterRequest( int cmd, bool on, std::list<CECPacket *> & request_list ) const {
   if( cmd == CMD_ID_SET_IPFILTER_CLIENTS_ON || cmd == CMD_ID_SET_IPFILTER_CLIENTS_OFF ) {
      cmd = CMD_ID_GET_IPFILTER_STATE_CLIENTS;
   }
   else if (cmd == CMD_ID_SET_IPFILTER_SERVERS_ON || cmd == CMD_ID_SET_IPFILTER_SERVERS_OFF) {
      cmd = CMD_ID_GET_IPFILTER_STATE_SERVERS;
   }
   else {
      cmd = CMD_ID_GET_IPFILTER_STATE;
   }
   CECPacket * request = new CECPacket( EC_OP_SET_PREFERENCES );
   CECEmptyTag prefs( EC_TAG_PREFS_SECURITY );
   if( cmd != CMD_ID_GET_IPFILTER_STATE_SERVERS ) {
      prefs.AddTag( CECTag( EC_TAG_IPFILTER_CLIENTS, (uint8)( on ? 1U : 0U )));
   }
   if(cmd != CMD_ID_GET_IPFILTER_STATE_CLIENTS ) {
      prefs.AddTag( CECTag( EC_TAG_IPFILTER_SERVERS, (uint8)( on ? 1U : 0U )));
   }
   request->AddTag( prefs );
   request_list.push_back( request );
   addGetIpFilterRequest( request_list );
}

void CamulecmdApp::addGetIpFilterLevelRequest( std::list<CECPacket *> & request_list ) const {
   CECPacket * request = new CECPacket( EC_OP_GET_PREFERENCES );
   request->AddTag( CECTag( EC_TAG_SELECT_PREFS, (uint32)EC_PREFS_SECURITY ));
   request_list.push_back( request );
}

int CamulecmdApp::addSetIpFilterLevelRequest( const wxString & args, std::list<CECPacket *> & request_list ) const {
   // This 'if' must stay as long as we support the deprecated 'IPLevel' command.
   if( ! args.IsEmpty()) {
   ...
   addGetIpFilterLevelRequest( request_list );
   return CMD_OK;
}

void CamulecmdApp::addPartFileRequest( const wxString & args, ECOpCodes opCode, std::list<CECPacket *> & request_list ) {
   if ( args.IsEmpty() ) {
   ...
   if( reply_all ) {
      CECPacket * request = new CECPacket( opCode );
      while( argsTokenizer.HasMoreTokens()) {
         wxString token = argsTokenizer.GetNextToken();
         ...
         if( token == wxT( "all" )) {
            ...
            for( CECPacket::const_iterator it = reply_all->begin(); it != reply_all->end(); ++it ) {
               const CEC_PartFile_Tag *tag = static_cast<const CEC_PartFile_Tag *>(&*it);
               wxString part_met_name = tag->PartMetName();
               if(  tag->FileName() == token
                  || part_met_name == token
                  || part_met_name.Truncate( part_met_name.Len() - 4 ) == token
                  || part_met_name.Truncate( part_met_name.Len() - 5 ) == token )
               {
                  Show(( "Processing by filename: " + token+wxT("\n")));
                  request->AddTag( CECTag( EC_TAG_PARTFILE, tag->FileHash()));
               }
            }
         }
...
void CamulecmdApp::addPriorityRequest( const wxString & argsIn, uint8 priority, std::list<CECPacket *> & request_list ) const {
   if( argsIn.IsEmpty()) {
      Show(_("This command requires an argument. Valid arguments: a file hash.\n"));
   }
   else {
      wxString args = argsIn;
      args = args.Trim( false ).Trim( true );
      CMD4Hash hash;
      if( hash.Decode( args )) {
         if( hash.IsEmpty()) {
            ...
            CECPacket * request = new CECPacket( EC_OP_PARTFILE_PRIO_SET );
            CECTag      hashtag( EC_TAG_PARTFILE, hash );
            hashtag.AddTag( CECTag( EC_TAG_PARTFILE_PRIO, priority ));
...
void CamulecmdApp::addAddLinkRequest( const wxString & argsIn, std::list<CECPacket *> & request_list ) const {
   wxString args = argsIn;
   ...
   CECPacket * request = new CECPacket( EC_OP_ADD_LINK );
   request->AddTag( CECTag( EC_TAG_STRING, args ));
   request_list.push_back( request );
}

void CamulecmdApp::addGetBWLimitRequest( std::list<CECPacket *> & request_list ) const {
   CECPacket * request = new CECPacket( EC_OP_GET_PREFERENCES );
   request->AddTag( CECTag( EC_TAG_SELECT_PREFS, (uint32)EC_PREFS_CONNECTIONS ));
   request_list.push_back( request );
}

int CamulecmdApp::addSetBWLimitRequest( const wxString & args, ECTagNames tagName, std::list<CECPacket *> & request_list ) const {
   int retVal = CMD_OK;
   unsigned long int limit;
   if( args.ToULong( &limit )) {
      CECPacket * request = new CECPacket( EC_OP_SET_PREFERENCES );
      CECEmptyTag prefs(EC_TAG_PREFS_CONNECTIONS );
      prefs.AddTag(CECTag( tagName, (uint16)limit ));
      request->AddTag(prefs);
      request_list.push_back(request);
      addGetBWLimitRequest( request_list );
   }
   else {
      retVal = CMD_ERR_INVALID_ARG;
   }
   return retVal;
}

int CamulecmdApp::addStatTreeRequest( const wxString & args, std::list<CECPacket *> & request_list ) const {
   int retVal = CMD_OK;
   CECPacket * request = new CECPacket( EC_OP_GET_STATSTREE );
   if( ! args.IsEmpty()) {
...
int CamulecmdApp::addDownloadRequest( const wxString & args, std::list<CECPacket *> & request_list ) {
   int retVal = CMD_OK;
   if( args.IsEmpty()) {
      retVal = CMD_ERR_MUST_HAVE_PARAM;
   }
   else {
      ...
      if(( args.ToULong( &id ) == true )&&( id < m_Results_map.size())) {
         ...
         CECPacket * request = new CECPacket(EC_OP_DOWNLOAD_SEARCH_RESULT);
...
int CamulecmdApp::ProcessCommand( int CmdId ) {
   ...
   int retVal = CMD_OK;
   switch( CmdId ) {
      case CMD_ID_STATUS                    : request_list.push_back( new CECPacket( EC_OP_STAT_REQ, EC_DETAIL_CMD )); break;
      case CMD_ID_SHUTDOWN                  : request_list.push_back( new CECPacket( EC_OP_SHUTDOWN )); break;
      case CMD_ID_CONNECT                   : addConnectRequest( args, request_list ); break;
      case CMD_ID_CONNECT_ED2K              : request_list.push_back( new CECPacket( EC_OP_SERVER_CONNECT )); break;
      case CMD_ID_CONNECT_KAD               : request_list.push_back( new CECPacket( EC_OP_KAD_START )); break;
      case CMD_ID_DISCONNECT                : request_list.push_back( new CECPacket( EC_OP_DISCONNECT )); break;
      case CMD_ID_DISCONNECT_ED2K           : request_list.push_back( new CECPacket( EC_OP_SERVER_DISCONNECT )); break;
      case CMD_ID_DISCONNECT_KAD            : request_list.push_back( new CECPacket( EC_OP_KAD_STOP )); break;
      case CMD_ID_RELOAD_SHARED             : request_list.push_back( new CECPacket( EC_OP_SHAREDFILES_RELOAD )); break;
      case CMD_ID_RELOAD_IPFILTER_LOCAL     : request_list.push_back( new CECPacket( EC_OP_IPFILTER_RELOAD )); break;
      case CMD_ID_RELOAD_IPFILTER_NET       : request_list.push_back( createRequest( EC_OP_IPFILTER_UPDATE, args )); break;
      case CMD_ID_SET_IPFILTER_ON           :
      case CMD_ID_SET_IPFILTER_CLIENTS_ON   :
      case CMD_ID_SET_IPFILTER_SERVERS_ON   : addSetIpFilterRequest( CmdId, true, request_list ); break;
      case CMD_ID_SET_IPFILTER_OFF          :
      case CMD_ID_SET_IPFILTER_CLIENTS_OFF  :
      case CMD_ID_SET_IPFILTER_SERVERS_OFF  : addSetIpFilterRequest( CmdId, false, request_list ); break;
      case CMD_ID_GET_IPFILTER              :
      case CMD_ID_GET_IPFILTER_STATE        :
      case CMD_ID_GET_IPFILTER_STATE_CLIENTS:
      case CMD_ID_GET_IPFILTER_STATE_SERVERS: addGetIpFilterRequest( request_list ); break;
      case CMD_ID_SET_IPFILTER_LEVEL        : retVal = addSetIpFilterLevelRequest( args, request_list ); break;
      case CMD_ID_GET_IPFILTER_LEVEL        : addGetIpFilterLevelRequest( request_list ); break;
      case CMD_ID_CANCEL                    : addPartFileRequest( args, EC_OP_PARTFILE_DELETE, request_list ); break;
      case CMD_ID_PAUSE                     : addPartFileRequest( args, EC_OP_PARTFILE_PAUSE , request_list ); break;
      case CMD_ID_RESUME                    : addPartFileRequest( args, EC_OP_PARTFILE_RESUME, request_list ); break;
      case CMD_ID_PRIORITY_LOW              : addPriorityRequest( args, (uint8)PR_LOW   , request_list ); break;
      case CMD_ID_PRIORITY_NORMAL           : addPriorityRequest( args, (uint8)PR_NORMAL, request_list ); break;
      case CMD_ID_PRIORITY_HIGH             : addPriorityRequest( args, (uint8)PR_HIGH  , request_list ); break;
      case CMD_ID_PRIORITY_AUTO             : addPriorityRequest( args, (uint8)PR_AUTO  , request_list ); break;
      case CMD_ID_SHOW_UL                   : request_list.push_back( new CECPacket( EC_OP_GET_ULOAD_QUEUE )); break;
      case CMD_ID_SHOW_DL                   : request_list.push_back( new CECPacket( EC_OP_GET_DLOAD_QUEUE )); break;
      case CMD_ID_SHOW_LOG                  : request_list.push_back( new CECPacket( EC_OP_GET_LOG )); break;
      case CMD_ID_SHOW_SERVERS              : request_list.push_back( new CECPacket( EC_OP_GET_SERVER_LIST, EC_DETAIL_CMD )); break;
      case CMD_ID_SHOW_SHARED               : request_list.push_back( new CECPacket( EC_OP_GET_SHARED_FILES )); break;
      case CMD_ID_RESET_LOG                 : request_list.push_back( new CECPacket( EC_OP_RESET_LOG )); break;
      case CMD_ID_ADDLINK                   : addAddLinkRequest   ( args,                     request_list ); break;
      case CMD_ID_SET_BWLIMIT_UP            : addSetBWLimitRequest( args, EC_TAG_CONN_MAX_UL, request_list ); break;
      case CMD_ID_SET_BWLIMIT_DOWN          : retVal = addSetBWLimitRequest( args, EC_TAG_CONN_MAX_DL, request_list ); break;
      case CMD_ID_GET_BWLIMITS              : addGetBWLimitRequest(                           request_list ); break;
      case CMD_ID_STATTREE                  : addStatTreeRequest  ( args,                     request_list ); break;
      case CMD_ID_SEARCH_GLOBAL             : addSearchRequest    ( args, EC_SEARCH_GLOBAL,   request_list ); break;
      case CMD_ID_SEARCH_LOCAL              : addSearchRequest    ( args, EC_SEARCH_LOCAL ,   request_list ); break;
      case CMD_ID_SEARCH_KAD                : addSearchRequest    ( args, EC_SEARCH_KAD   ,   request_list ); break;
      case CMD_ID_RENAME                    : addRenameRequest    ( args,                     request_list ); break;
      case CMD_ID_SEARCH                    : Show(_("No search type defined.\nType 'help search' to get more help.\n")); break;
      case CMD_ID_SEARCH_RESULTS            : request_list.push_back(new CECPacket(EC_OP_SEARCH_RESULTS, EC_DETAIL_FULL)); break;
      case CMD_ID_SEARCH_PROGRESS           : request_list.push_back(new CECPacket(EC_OP_SEARCH_PROGRESS)); break;
      case CMD_ID_DOWNLOAD                  : retVal = addDownloadRequest( args, request_list ); break;
      default                               : retVal = CMD_ERR_PROCESS_CMD; break;
   }
   if( retVal == CMD_OK ) {
      m_last_cmd_id = CmdId;
      if( ! request_list.empty()) {
         std::list<CECPacket *>::iterator it = request_list.begin();
         while(( retVal == CMD_OK )&&( it != request_list.end())) {
            CECPacket *curr = *it++;
...
void CamulecmdApp::OnInitCommandSet()
{
   ...
   m_commands.AddCommand(wxT("Add"), CMD_ID_ADDLINK, wxTRANSLATE("Add an eD2k or magnet link to core."),
               wxTRANSLATE("The eD2k link to be added can be:\n*) a file link (ed2k://|file|...), it will be added to the download queue,\n*) a server link (ed2k://|server|...), it will be added to the server list,\n*) or a server list link, in which case all servers in the list will be added to the\n   server list.\n\nThe magnet link must contain the eD2k hash and file length.\n"), CMD_PARAM_ALWAYS);
