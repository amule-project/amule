...
uint8 CClientCreditsList::CreateSignature(CClientCredits* pTarget, mule_byte* pachOutput, uint8 nMaxSize, uint32 ChallengeIP, uint8 byChaIPKind, void* sigkey)
{
	...
	try {
		...
		CryptoPP::AutoSeededX917RNG<CryptoPP::DES_EDE3> rng;
		mule_byte abyBuffer[MAXPUBKEYSIZE+9];
...
bool CClientCreditsList::VerifyIdent(CClientCredits* pTarget, const mule_byte* pachSignature, uint8 nInputSize, uint32 dwForIP, uint8 byChaIPKind)
{
	wxASSERT( pTarget );
	...
	bool bResult;
	try {
		CryptoPP::StringSource ss_Pubkey((mule_byte*)pTarget->GetSecureIdent(),pTarget->GetSecIDKeyLen(),true,0);
