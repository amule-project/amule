...
CTag *CFileDataIO::ReadTag(bool bOptACP) const
{
	...
	try {
		...
		switch (type)
		{
			...
			//
			// And still.. it doesn't't work this way without breaking backward compatibility. To properly
			...
			// the net in some months.
			//
			// And still.. it doesn't't work this way without breaking backward compatibility
			case TAGTYPE_BSOB:
			{
				uint8 size = 0;
