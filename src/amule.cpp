	...
	if (result == 1) {
		...
		if (!file.Open(filename)) {
			...
			if (curVer < newVer) {
				...
				#ifdef AMULE_DAEMON
				puts(unicode2UTF8(wxString::Format(_("WARNING: Your aMuled version is outdated: %i.%i.%i < %i.%i.%i"), VERSION_MJR, VERSION_MIN, VERSION_UPDATE, fields[0], fields[1], fields[2])));
				#endif
			} else {
